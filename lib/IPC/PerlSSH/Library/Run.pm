...
sub system_inout {
   ...
   while(1) {
      ...
      last unless $wr0 or $rd1;
      my $wv = ''; vec( $wv, $fn0, 1 ) = 1 if $wr0;
      my $rv = ''; vec( $rv, $fn1, 1 ) = 1 if $rd1;
...
func system => q{ ( system_inout( "", @_ ) )[0] };
...
func system_in => q{ ( system_inout( @_ ) )[0] };
...
func system_out => q{ system_inout( "", @_ ) };
...
and returning its exit status and what it wrote on STDOUT.
 ...
 my ( $exitstatus, $stdout ) = $ips->call( "system_inout", $stdin, $path, @args )

=cut

func system_inout => q{ system_inout( @_ ) };
